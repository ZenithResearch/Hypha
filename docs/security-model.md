...
- A room repository attachment publishes only its repository name, remote repository URL, and fixed `out/`/`out.json` contract as Matrix room state. Local filesystem paths, security-scoped bookmarks, and build commands remain device-local and are never placed in Matrix events.
- Local repository access is user-selected and security-scoped. A build command is optional and may be entered locally or read from the local checkout's `out/out.json`; Hypha executes either source only after explicit confirmation and runs it from the selected repository root with the user's own permissions. Matrix room state can never supply or trigger a command.
