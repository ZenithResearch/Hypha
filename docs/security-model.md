...
- Private GitHub access verification accepts a fine-grained personal access token only in a hidden field, rejects malformed token material and non-GitHub/insecure remotes before networking, sends the token only as an HTTPS `api.github.com` authorization header, rejects redirects, uses no cookies/cache/credential store, clears UI token state before awaiting the request, and never persists or publishes the token. Verification does not silently clone, fetch, or pull the local checkout.
